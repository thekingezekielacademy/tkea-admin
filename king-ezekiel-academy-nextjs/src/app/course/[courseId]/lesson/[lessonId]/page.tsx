...
const LessonPlayer: React.FC = () => {
  ...
  // Fetch course and lesson data function
    ...
    if (!courseId || !lessonId) return;
      ...
      try {
        // Only show loading when no course data exists - prevents refresh appearance
        if (!course || !currentVideo) {
          setLoading(true);
        }
        setError(null);
      
      // Skip session refresh to prevent navigation refresh issues
      ...
      try {
        localStorage.setItem('recent_course_id', courseId);
  ...
  const handleVideoEnded = async () => {
    ...
    try {
      if (user?.id) {
        try {
          ...
          if (!progressError) {
            ...
            // Import and use the course progress service
            const { CourseProgressService } = await import('@/services/courseProgressService');
            await CourseProgressService.onLessonCompleted(user.id, courseId, lessonId);
      ...
      try {
        const currentProgress = completedLessons.size + 1;
  ...
  const nextLesson = () => {
    if (course?.course_videos && currentLessonIndex < course.course_videos.length - 1) {
      ...
      router.push(`/course/${courseId}/lesson/${nextVideo.id}`);
    }
  };
  ...
  const prevLesson = () => {
    if (course?.course_videos && currentLessonIndex > 0) {
      ...
      router.push(`/course/${courseId}/lesson/${prevVideo.id}`);
    }
  };
  ...
  }, [courseId, lessonId]); // Removed user?.id to prevent refresh on navigation return
                ...
                {course.course_videos?.sort((a: any, b: any) => a.order_index - b.order_index).map((video: any, index: number) => {
                  ...
                  return (
                    <div 
                      ...
                      onClick={() => router.push(`/course/${courseId}/lesson/${video.id}`)}
                    >
