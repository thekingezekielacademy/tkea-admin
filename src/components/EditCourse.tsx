...
interface PDFResource {
  ...
  file_path?: string;
  file_size?: number;
  isNew?: boolean; // To track if it's a new upload or existing
}
...
const EditCourse: React.FC = () => {
  ...
  const fetchCourseData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch course PDF resources
      const { data: pdfResources, error: resourcesError } = await supabase
      ...
      if (resourcesError) {
        console.warn('Error fetching PDF resources:', resourcesError);
        // Don't throw, just continue without resources
      }
      ...
      console.log('📚 Loaded course data:', {
        ...
        pdfResourcesCount: pdfResources?.length || 0
      });

      // Transform PDF resources to match our interface
      const transformedPdfResources: PDFResource[] = (pdfResources || []).map((resource: any) => ({
        id: resource.id,
        name: resource.name,
  ...
  const handleUpdateCourse = async () => {
    try {
      ...
      if (courseData.pdfResources.length > 0) {
        try {
          // Get existing PDF resources from database
          const { data: existingResources, error: fetchResourcesError } = await supabase
            .from('course_resources')
            .select('*')
            .eq('course_id', courseId);

          if (fetchResourcesError) {
            console.warn('Error fetching existing PDF resources:', fetchResourcesError);
          }

          const existingResourceIds = new Set((existingResources || []).map((r: any) => r.id));
          const currentResourceIds = new Set(
            courseData.pdfResources
              .filter(r => !r.isNew)
              .map(r => r.id)
          );

          // Delete removed PDF resources
          const resourcesToDelete = (existingResources || []).filter(
            (r: any) => !currentResourceIds.has(r.id)
          );
          ...
          for (const resourceToDelete of resourcesToDelete) {
            try {
              ...
              console.warn(`Error deleting PDF resource ${resourceToDelete.id}:`, deleteErr);
            }
          }

          // Upload new PDF resources
          const newResources = courseData.pdfResources.filter(r => r.isNew && r.file);
          const pdfResourcesToInsert = [];

          for (const pdfResource of newResources) {
            if (!pdfResource.file) continue;
          ...
          // Don't throw error, course is already updated
        }
      } else {
        // If no PDFs in courseData, delete all existing PDFs
        try {
          ...
          console.warn('Error deleting all PDF resources:', deleteErr);
        }
      }
        ...
        <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-lg shadow-xl p-8 mb-6 border-4 border-indigo-400">
          ...
          {courseData.pdfResources.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Current PDFs ({courseData.pdfResources.length})</h3>
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      {pdf.file ? (
                        <p className="text-sm text-gray-500">{(pdf.file.size / 1024 / 1024).toFixed(2)} MB (new upload)</p>
                      ) : pdf.file_size ? (
                        <p className="text-sm text-gray-500">{(pdf.file_size / 1024 / 1024).toFixed(2)} MB</p>
                      ) : null}
                    </div>
                  </div>
                  <button
