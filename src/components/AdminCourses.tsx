...
const AdminCourses: React.FC = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      if (filterStatus === 'scheduled') {
        query = query.eq('is_scheduled', true).eq('status', 'scheduled');
      } else if (filterStatus === 'published') {
        query = query.eq('status', 'published');
      }
      // 'all' shows everything, so no additional filter

      const { data: coursesData, error: queryError } = await query;
      ...
      const transformedCourses: Course[] = (coursesData || []).map(course => {
        ...
        if (course.course_videos && course.course_videos.length > 0) {
          const totalSeconds = course.course_videos.reduce((total: number, video: any) => {
        ...
        return {
          ...
          totalDuration,
          is_scheduled: course.is_scheduled || false,
          status: course.status || 'published',
          scheduled_for: course.scheduled_for || undefined
        };
      });
  ...
  const getStatusBadge = (course: Course) => {
    if (course.is_scheduled && course.status === 'scheduled') {
      const scheduledDate = course.scheduled_for ? new Date(course.scheduled_for) : null;
